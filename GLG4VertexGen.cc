...
////////////////////////////////////////////////////////////////
const char * GLG4VertexGen_Gun::fTheElementNames[] = {
  "H",                                                                                                "He", 
  "Li", "Be",                                                             "B",  "C",  "N",  "O", "F", "Ne", 
...
GLG4VertexGen_Gun::GLG4VertexGen_Gun(const char *arg_dbname)
  : GLG4VertexGen(arg_dbname), fMom(0.,0.,0.), fKe(0.0), fPol(0.,0.,0.),
    fMultiplicity(1)
{
...
{
  int max;
  if (fRandom==1){max=(int)(fMultiplicity*G4UniformRand()+1);}//!explain in heder
  else{max=fMultiplicity;}
   
  for (int imult=0; imult<max; imult++) {
...
SetState(G4String newValues)
{
  ...
  db[ (fDbname+".pdgcode").c_str() ] = fPDef->GetPDGEncoding();
  ...
  is >> x;
  if (is.fail()) {
     db[ (fDbname+".px").c_str() ] = fMom.x();
     db[ (fDbname+".py").c_str() ] = fMom.y();
     db[ (fDbname+".pz").c_str() ] = fMom.z();
     db[ (fDbname+".KE").c_str() ] = fKe;
     return;
  }
  if (x <= 0.0) {
     p_renorm = 0.0;
  ...
  db[ (fDbname+".polx").c_str() ] = fPol.x();
  db[ (fDbname+".poly").c_str() ] = fPol.y();
  db[ (fDbname+".polz").c_str() ] = fPol.z();
...
GetState()
{
  ...
  os << fPDef->GetParticleName() << '\t'
     ...
     << fMultiplicity << ' '<< fRandom << G4std::ends;
  G4String rv(os.str());
  return rv;
}
...
SetState(G4String newValues)
{
  ...
  db[ (fDbname+".py").c_str() ] = fMom.y();
  db[ (fDbname+".pz").c_str() ] = fMom.z();
  db[ (fDbname+".KE1").c_str() ] = fKe1;
  db[ (fDbname+".KE2").c_str() ] = fKe2;
...
////////////////////////////////////////////////////////////////   Gun V3
const char * GLG4VertexGen_Gun3::fTheElementNames[] = {
  "H",                                                                                                "He", 
  "Li", "Be",                                                             "B",  "C",  "N",  "O", "F", "Ne", 
  "Na", "Mg",                                                             "Al", "Si", "P", "S", "Cl", "Ar", 
  "K", "Ca", "Sc", "Ti", "V", "Cr", "Mn", "Fe", "Co", "Ni", "Cu", "Zn", "Ga", "Ge", "As", "Se", "Br", "Kr", 
...
GLG4VertexGen_Gun3::GLG4VertexGen_Gun3(const char *arg_dbname)
    : GLG4VertexGen(arg_dbname), fMom(0.,0.,0.), fKe1(0.0), fKe2(0.0), fPol(0.,0.,0.),
    fMultiplicity(1) 
{
  fPDef= G4ParticleTable::GetParticleTable()->FindParticle("geantino");
...
GLG4VertexGen_Gun3::~GLG4VertexGen_Gun3()
{
}
...
/** Generates one or more particles with a specified momentum, or isotropic
    momentum with specific energy, and specified polarization, or uniformly
    random polarization, based on parameters set via SetState() */
void GLG4VertexGen_Gun3::
GeneratePrimaryVertex(G4Event *argEvent, G4ThreeVector &dx, G4double dt)
{
  GLG4param &db ( GLG4param::GetDB() );
 
  G4double temp_mom_x, temp_mom_y, temp_mom_z, temp_time, temp_ke;
  G4double temp_norm_mom_x, temp_norm_mom_y, temp_norm_mom_z;
  G4double temp_mag;
  temp_norm_mom_x = db[ (fDbname+".px").c_str() ];
  temp_norm_mom_y = db[ (fDbname+".py").c_str() ];
  temp_norm_mom_z = db[ (fDbname+".pz").c_str() ];
  temp_ke = db[(fDbname+".KE").c_str()];
  temp_time = db[ (fDbname+".time").c_str() ];

  cout<<"normalized momentum "<<temp_norm_mom_x<<"  "<<temp_norm_mom_y<<"  "<<temp_norm_mom_z<<"  "<<temp_ke<<endl;

  //  _mom= G4ThreeVector(temp_mom_x, temp_mom_y, temp_mom_z);
  dt = temp_time;
  ...
  temp_mag = sqrt(temp_norm_mom_x*temp_norm_mom_x + temp_norm_mom_y*temp_norm_mom_y + temp_norm_mom_z*temp_norm_mom_z);

  temp_mom_x = temp_norm_mom_x*sqrt( temp_ke*(temp_ke+2*mass) /temp_mag);
  temp_mom_y = temp_norm_mom_y*sqrt( temp_ke*(temp_ke+2*mass) /temp_mag);
  temp_mom_z = temp_norm_mom_z*sqrt( temp_ke*(temp_ke+2*mass) /temp_mag);

  cout<<temp_mom_x<<" "<<temp_mom_y<<" "<<temp_mom_z<<endl;

  particle=
    new G4PrimaryParticle(fPDef,              // particle code
			  temp_mom_x,           // x component of momentum
			  temp_mom_y,           // y component of momentum
			  temp_mom_z,
			  temp_ke);   // z component of momentum
  //dir = dir.unit();
  ...
  cout<<"The energy is "<<temp_ke<<" the momentum is "<<temp_mag<<" "<<temp_mom_x<<"  "<<temp_mom_y<<" "<<temp_mom_z<<"  "<<sqrt(temp_mom_x*temp_mom_x + temp_mom_y*temp_mom_y + temp_mom_z*temp_mom_z)<<endl;
...
/** Set state of the GLG4VertexGen_Gun, or show current state and
    ...
    Format of argument to GLG4VertexGen_Gun::SetState:
       "pname  momx_MeV momy_MeV momz_MeV  KE_MeV time "
...
SetState(G4String newValues)
{
  ...
  if (newValues.length() == 0) {
     ...
     G4cout << "Current state of this GLG4VertexGen_Gun:\n"
	         << " \"" << GetState() << "\"\n" << G4endl;
     G4cout << "Format of argument to GLG4VertexGen_Gun::SetState: \n"
               " \"pname  momx_MeV momy_MeV momz_MeV  KE_MeV time_ns\"\n"
               " where pname is the name of a particle type (e-, mu-, U238, ...)\n"
               " mom*_MeV is a momentum in MeV/c\n"
               " KE_MeV is the kinetic energy\n"
               " time_ns is time in ns\n"
               "mom*_MeV==0  --> random isotropic directions chosen\n"
	         << G4endl;
     return;
  }
  ...
  db[ (fDbname+".KE1").c_str() ] = fKe1;
  db[ (fDbname+".KE2").c_str() ] = fKe2;
  db[ (fDbname+".time").c_str() ] = fTime;

}


G4String GLG4VertexGen_Gun3::
GetState()
{
  G4std::ostringstream os;
  ...
  for (;;) {
      ...
      if ( strcmp(firstword, "STATE") == 0 ) {
         ...
         while ( fgets(buffer, size, fFile) == buffer ) {
	            ...
	            if (nscan == 2)
	               db[ ((fDbname+".")+firstword).c_str() ]= value;
	            else {
